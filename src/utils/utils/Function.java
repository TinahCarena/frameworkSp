...
import controller.AnnotationMapping;
...
public class Function {
    ...
    public HashMap<String, Mapping> scanControllersMethods(List<String> controllers) throws Exception {
        ...
        for (String c : controllers) {
            ...
            for (Method method : meths) {
                if (method.isAnnotationPresent(AnnotationMapping.class)) {
                    String url = method.getAnnotation(AnnotationMapping.class).url();
    ...
    public static Object[] getParameterValue(HttpServletRequest request, Method method,
            Class<AnnotationReqParam> annotationClass,
            Class<AnnotationParamObject> paramObjectAnnotationClass) throws Exception {
        ...
        for (int i = 0; i < parameters.length; i++) {
            if (parameters[i].getType().equals(MySession.class)) {
                ...
                AnnotationReqParam param = parameters[i].getAnnotation(annotationClass);
                ...
                AnnotationParamObject paramObject = parameters[i].getAnnotation(paramObjectAnnotationClass);
                ...
                throw new Exception("ETU002704 : tsy misy annotation");
            }
        }
